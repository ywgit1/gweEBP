...
from xfr.inpainting_game.generate_blackbox_saliency_UoM import generate_bb_smaps
        ...
        def bb_fn(probes, gallery):
            ...
            if isinstance(probes[0], np.ndarray):
                ...
                if probes[0].shape[2] == 3:
                    for probe in tqdm(probes, desc='Processed probes'):
                        probe = wb.convert_from_numpy(probe)[0] # This will consume too many resources!!
                        probe_vec = wb.embeddings([probe])
                        probe_vecs.append(probe_vec)
                    probe_vecs = np.concatenate(probe_vecs, axis=0)
            else:
...
def run_experiments(params):
    ...
    if params['SUBJECT_ID'] is None:
        ...
        params['SUBJECT_ID'] = ["{:05}".format(sid) for sid in params['SUBJECT_ID']]

    def valid_imgnums(net_ds, params_):
        uom_ds = net_ds[params_['WB_NET'][0]]
        subset = uom_ds.loc[
            (uom_ds['SUBJECT_ID'] == int(params_['SUBJECT_ID'][0])) &
            (uom_ds['MASK_ID'] == int(params_['MASK_ID'][0])) &
            (uom_ds['TRIPLET_SET'] == 'PROBE')
        ]
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--perct', nargs='+', dest='MASKING_PERCT',
        # default=['6', '12', '18', '24', '32'],
        default=[10],
        type=int,
        help='Percentage of the masking cells',
        )
    parser.add_argument(
