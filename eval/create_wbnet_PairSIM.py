...
from xfr.models import whitebox as whitebox 
...
def create_wbnet(net_name, device):
    ...
    if net_name == 'resnetv6_pytorch':
        ...
        from xfr.models import vgg_PairSIM
        param_path = os.path.join(
            xfr_root, 'models/vgg_tri_2_Mon_11Oct2021_221223_epoch30.pth')

        net = vgg_PairSIM.get_model(param_path)
        net.to(device)
        ...
        from xfr.models import lcnn9_PairSIM
        net = lcnn9_PairSIM.get_model(param_path)
        net.eval()
        net.to(device)
