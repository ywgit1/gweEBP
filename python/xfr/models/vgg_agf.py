...
class VggEmbeddingNet(torch_nn.Module):
    ...
    def AGF(self, **kwargs):
        ...
        cams, grad_outputs = self.maxpool.AGF(cam, grad_outputs, **kwargs)
        cam_list = list(reversed(cams)) + cam_list        
        cams, grad_outputs = self.basenet_part2.AGF(cams[-1], grad_outputs, **kwargs)
        cam_list = list(reversed(cams)) + cam_list
        cams, grad_outputs = self.basenet_part1.AGF(cams[-1], grad_outputs, **kwargs)
        cam_list = list(reversed(cams)) + cam_list
