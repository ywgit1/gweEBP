...
from xfr.models.RSP import layers as nn
import os
import torchvision.transforms as transforms
import torch.nn.functional as F

class Vgg_face_dag(torch_nn.Module):
...
class VggEmbeddingNet(torch_nn.Module):
    ...
    def forward(self, x):        
        ...
        # x = self.basenet_part2(x)
        x = self.features(x)
        x1 = x.view(x.size()[0], -1) # Flatten
        emd = self.fc(x1)
        # emd = F.normalize(emd, p=2, dim=1)
        self.emd_norm = torch.norm(emd, p=2, dim=1).detach().clone()
        x = self.fc2(emd)
        
        return x, emd  # To be consistent with the solver class # unflattened feature map, embedding
    ...
    #     x = self.maxpool.relprop(x, alpha)
    #     x = self.basenet_part2.relprop(x, alpha)
    #     x = self.basenet_part1.relprop(x, alpha)

        return x

    def RSP(self, R):
        R_list = self.features[:-1].RSP_relprop(R) # exclude the last maxpool layer
    ...
    def reset(self):
        ...
        self.features = nn.Sequential(*(layers_part1 + layers_part2))
...
def get_model(vgg_tri_2_pth=None):
    ...
    model.basenet_part1 = None
    model.basenet_part2 = None
    return model
