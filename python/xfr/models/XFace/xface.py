...
class XFace:
    ...
    def __init__(self, model, inference_func):
        self.model = model
    ...
    def __call__(self, img1, img2):
        ...
        # )
        # model.to(device)
        mapGenerator = MapGenerator(inference_fn=self.inference_func, model=self.model)  # LCNN9
        ...
        if isinstance(img1, str):
            img1 = cv2.cvtColor(cv2.imread(img1), cv2.COLOR_BGR2RGB).astype(np.float32) / 255
        if isinstance(img2, str):
            img2 = cv2.cvtColor(cv2.imread(img2), cv2.COLOR_BGR2RGB).astype(np.float32) / 255
        image_pair = (
            img1, img2
        )
