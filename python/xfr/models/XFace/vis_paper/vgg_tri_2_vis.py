...
class VggEmbeddingNet(nn.Module):
    ...
    def forward(self, x):
        ...
        emd = self.fc(x1)
        # emd = F.normalize(emd, p=2, dim=1)  # Yan
...
def get_embedding(img: np.array, model: nn.Module):
    ...
    class C(object):
        meta = {'mean': [129.186279296875, 104.76238250732422, 93.59396362304688],
                ...
                'imageSize': [224, 224, 3],
                'multiplier': 255.0}
        data_transform = {'img_resize': 256, 'crop_type': 0,  # 0: no crop, 1: centre_crop, 2: random_crop
                          'random_flip': False,
                          'override_meta_imsize': False,
                          'to_grayscale': False}
...
if __name__ == '__main__':
    ...
    from PIL import Image
    qimg = _get_data_transforms()(Image.open('../demo/img1.png'))
    qimg = qimg.unsqueeze(0)
    qimg_t = qimg.to(device)
    emd_query = model(qimg_t)
