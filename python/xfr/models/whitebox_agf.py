...
from xfr.models.AGF.layers import Linear
...
class WhiteboxLightCNN9(WhiteboxNetwork):
    ...
    def restore_emd_layer(self, device=None):
        from xfr.models.lcnn9_agf import mfm
        if device is None:
...
class Whitebox(nn.Module):
    ...
    def AGF(self, img_probe, **kwargs):
        ...
        R_mate = self.net.net.AGF(class_id=[0], **kwargs)
        i_layer = -4
        if 'i_layer' in kwargs:
            i_layer = kwargs['i_layer']
        R_mate = R_mate[i_layer]
        R_mate = self._to_saliency(np.squeeze(R_mate.detach().cpu().numpy().transpose((0, 2, 3, 1))))
        R_nonmate = self.net.net.AGF(class_id=[1], **kwargs)
        R_nonmate = R_nonmate[i_layer]
        R_nonmate = self._to_saliency(np.squeeze(R_nonmate.detach().cpu().numpy().transpose((0, 2, 3, 1))))
        R = np.maximum(R_mate - R_nonmate, 0)
        # R = self._to_saliency(np.squeeze(R.detach().cpu().numpy().transpose((0, 2, 3, 1))))
