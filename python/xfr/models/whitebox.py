...
class WhiteboxLightCNN9(WhiteboxNetwork):
    ...
    def _layer_visitor(self, f_forward=None, f_preforward=None, net=None):
        ...
        for name, layer in net._modules.items():
            hooks = []
            if isinstance(layer, nn.Sequential) or isinstance(layer, Bottleneck) or \
                isinstance(layer, xfr.models.lightcnn.mfm) or \
                isinstance(layer, xfr.models.lightcnn.group) or \
                isinstance(layer, xfr.models.lightcnn.resblock):
                layerlist.append({'name': str(layer), 'hooks': [None]})
                layerlist = layerlist + self._layer_visitor(f_forward, f_preforward, layer)
            else:
                if f_forward is not None:
...
class Whitebox(nn.Module):
    ...
    def contrastive_ebp(self, img_probe, k_poschannel, k_negchannel):
        ...
        # Mated EBP
        #img_probe = img_probe.expand(2, *img_probe.shape[1:])
        P0 = torch.zeros( (img_probe.shape[0], self.net.num_classes()) );  P0[:, k_poschannel] = 1.0;  # one-hot
        ...
        # Non-mated EBP
        P0 = torch.zeros( (img_probe.shape[0], self.net.num_classes()) );  P0[:, k_negchannel] = 1.0;  # one-hot
        P0 = P0.to(img_probe.device)
