...
class Split(nn.RelPropSimple):
    ...
    def RSP_relprop(self, R):
        # Gradient
        if type(R) not in (tuple, list):
            raise ValueError('Expect R to be a list or tuple')
        R = torch.cat(R, dim=1)
        return R   

class Max(nn.RelPropSimple):
...
class mfm(torch_nn.Module):
    ...
    def RSP_relprop(self, R):
        R = self.max.RSP_relprop(R)
        R = self.split.RSP_relprop(R)
        R = self.filter.RSP_relprop(R)
        return R
        
class group(nn.RelPropSimple):
    ...
    def RSP_relprop(self, R):
        R = self.conv.RSP_relprop(R)
        R = self.conv_a.RSP_relprop(R)
        return R
