...
from xfr.models.RSP.layers import Linear
from xfr.models.RSP.baselines.gradcam import GradCam
...
from xfr.models.vgg_rsp import vgg16_preprocess
...
class WhiteboxLightCNN9(WhiteboxNetwork):
    ...
    def restore_emd_layer(self, device=None):
        from xfr.models.lcnn9_rsp import mfm
        if device is None:
    ...
    def set_triplet_classifier(self, x_probe, x_mate, x_nonmate):
        # <editor-fold desc="[+] Original implementation ..."> # not merge layer
        device = next(self.net.parameters()).device
        self.net.fc2 = Linear(256, 2, bias=False).to(device)
        ...
        index = (1 - indicator) * r + indicator * (r + 256)
        index = index.long()
        self.net.fc = Linear(fc_weight.shape[1], 256, bias=True)
...
class WhiteboxVGG16(WhiteboxNetwork):
    ...
    def set_triplet_classifier(self, x_probe, x_mate, x_nonmate):
        # <editor-fold desc="[+] Original implementation ..."> # not merge layer
        device = next(self.net.parameters()).device
        self.net.fc2 = Linear(1024, 2, bias=False).to(device) # Linear layer that supports AGF
        self.net.fc2.weight.data.copy_(torch.cat( (x_mate, x_nonmate), dim=0) / self.net.emd_norm)
        self.net.reset_classifier()
...
class Whitebox(nn.Module):
    ...
    def _to_saliency(self, R, blur_radius=2):
    ...
    def RSP(self, img_probe, i_layer=-6, gradcam_layer=29):
        x = img_probe.detach().clone()
        y = self.net.classify(x).detach().cpu().numpy() # Obtain non-negative activations in a forward pass A_{n}
        assert np.all(y[:, 0] > y[:, 1])
        
        bp = GradCam(self.net.net, target_layer=gradcam_layer)
        x.requires_grad_(True)
        self.net.net.zero_grad()
        cam_tmp = bp.generate_cam_without_sum(x, target_class=0)
        cam_tmp = cam_tmp.detach().cpu().numpy()
        cam_tmp = np.maximum(cam_tmp, 0)
        cam_tmp = cam_tmp / (cam_tmp.max() + 1e-9)
        rel = torch.tensor(cam_tmp).to(x.device)
        R_mate = self.net.net.RSP(R=rel)[i_layer]
        
        self.net.net.zero_grad()
        cam_tmp = bp.generate_cam_without_sum(x, target_class=1)
        cam_tmp = cam_tmp.detach().cpu().numpy()
        cam_tmp = np.maximum(cam_tmp, 0)
        cam_tmp = cam_tmp / (cam_tmp.max() + 1e-9)
        rel = torch.tensor(cam_tmp).to(x.device)
        R_nonmate = self.net.net.RSP(R=rel)[i_layer]     
        
        R_mate = self._to_saliency(R_mate.detach().cpu().numpy())
        R_nonmate = self._to_saliency(R_nonmate.detach().cpu().numpy())
        R = np.maximum(R_mate - R_nonmate, 0)
        # R = R_mate
        R = R / R.sum()
        R = np.array(PIL.Image.fromarray(R).resize((img_probe.shape[3], img_probe.shape[2])))
