...
def make_inpaintinggame_plots(net_dict, params, human_net_labels):
    ...
    for base_net, numTripletsMask in numTriplets.items():
        ...
        x = np.arange(len(numTripletsMask))
    ...
    smap_pattern = os.path.join(
        smap_root,
        '{METHOD_LABEL}-{NET}/{SUBJECT_ID}/{ORIG_MASK_ID}-{METHOD}-saliency.npz'
    )
    ...
    for keys, grp in nonmate_classification.groupby(['NET', 'MASK_ID', 'METHOD']):
        for row_num, (idx, row) in enumerate(grp.iterrows()):
            ...
            gt_mask = resize(gt_mask, smap.shape)
            ...
            fpath = os.path.join(
                ...
                row['METHOD'],
                '%s-%d-idflip.png' % (
                    os.path.split(row['ORIGINAL_BASENAME'])[-1].replace('/', '-'),
                    row['ORIG_MASK_ID']
                ))
...
def plot_cls_vs_fpr(ax, grp, hnet, label,
    ...
    cls_at_fpr = dict()
    for target in [1e-2, 5e-2, 1e-1, 2e-1, 5e-1]:
...
def run_inpaintinggame_analysis(hgame_thresholds, hgame_percentile, params,
    ...
    smap_pattern = os.path.join(
        ...
        '{METHOD_LABEL}-{NET}/{SUBJECT_ID}/{MASK_ID}-{METHOD}-saliency.npz'
    )
    ...
    if params['SUBJECT_ID'] is None:
        net0 = list(inpainting_v2_data.keys())[0]
        params['SUBJECT_ID'] = inpainting_v2_data[net0]['SUBJECT_ID'].unique().tolist() 
    ...
    for net_name in params['NET']:
        ...
        for (subject_id, mask_id), ip2grp in subjs_net_inp.groupby(
            ...
            print(f"SUBJECT_ID={subject_id}, MASK_ID={mask_id}")
            ...
            mate_embeds = snet.embeddings(
                [os.path.join(inpaintgame3_dir, fn) for fn in ip2ref['OriginalFile']]
            )
            ...
            nonmate_embeds = snet.embeddings(
                [os.path.join(inpaintgame3_dir, fn) for fn in ip2ref['InpaintingFile']]
            )
            ...
            original_imITF = snet.preprocess_loader(
                [os.path.join(inpaintgame3_dir, fn) for fn in ip2probe['OriginalFile']]
            )
            inpaint_imITF = snet.preprocess_loader(
                [os.path.join(inpaintgame3_dir, fn) for fn in ip2probe['InpaintingFile']]
            )
            ...
            for (
                ...
                for method, suffix_aggr in itertools.product(
                    ...
                    def calc_twin_cls():
                        ...
                        print('SMAP MAX VALUE={}'.format(smap.max()))
                    ...
                    cache_fn = (
                        ...
                        '{THRESHOLDS}{ZERO_SALIENCY_SUFFIX}'
                            ...
                            ORIGINAL_BASENAME=os.path.split(row['ORIGINAL_BASENAME'])[-1],
                            METHOD=method,
                            NET=net,
                    ...
                    def calc_saliency_intersect_over_union():
                        ...
                        # Resize the mask to the same size of the saliency map
                        inpainted_region = resize(inpainted_region, smap.shape)
...
def generate_plots(nonmate_classification, hgame_thresholds, hgame_percentile,
    ...
    cls_at_fpr_method_msk = defaultdict(dict) # nonmate cls. rate @ FPR for each mask
    ...
    for method, cls_at_fpr_maskid in  cls_at_fpr_method_msk.items():
        nrow = dict()
        print(method)
        print('\tOverall\t%0.6f\t%0.6f\t%0.6f\t%0.6f\t%0.6f' % (
            cls_at_fpr_method[method][1e-2],
            cls_at_fpr_method[method][5e-2],
            cls_at_fpr_method[method][1e-1],
            cls_at_fpr_method[method][2e-1],
            cls_at_fpr_method[method][5e-1]
        ))
        nrow['method'] = method
        nrow['all,far=1e-2'] = cls_at_fpr_method[method][1e-2]
        nrow['all,far=5e-2'] = cls_at_fpr_method[method][5e-2]
        nrow['all,far=1e-1'] = cls_at_fpr_method[method][1e-1]
        nrow['all,far=2e-1'] = cls_at_fpr_method[method][2e-1]
        nrow['all,far=5e-1'] = cls_at_fpr_method[method][5e-1]

        # for mask_id in [0, 1, 2, 3, 4, 5]:
        for mask_id, cls_at_fpr in cls_at_fpr_maskid.items():
            # cls_at_fpr = cls_at_fpr_maskid[mask_id]
            print('\t%d\t%0.6f\t%0.6f\t%0.6f\t%0.6f\t%0.6f \t(%s)' % (
                mask_id,
                cls_at_fpr[1e-2],
                cls_at_fpr[5e-2],
                cls_at_fpr[1e-1],
            ...
            nrow['%s,far=1e-1' % regions_human_labels[mask_id]] = cls_at_fpr[1e-1]
            nrow['%s,far=2e-1' % regions_human_labels[mask_id]] = cls_at_fpr[2e-1]
            nrow['%s,far=5e-1' % regions_human_labels[mask_id]] = cls_at_fpr[5e-1]            
