...
import test_whitebox as xfr_fn
...
def run_contrastive_triplet_ebp(wb, im_mates, im_nonmates, probe_im,
    ...
    if truncate_percent is None:
        ...
        # <editor-fold desc="[+] LCNN9 ...">
        probe, mate, nonmate = param_package
        if net_name.lower() == "vgg16":
            cam1, hmap1 = xfr_fn.contrastive_triplet_ebp_vgg16_vmeval(wb, probe, mate, nonmate, device)
        elif net_name.lower() == "lcnn9":
            cam1, hmap1 = xfr_fn.contrastive_triplet_ebp_lightcnn9_vmeval(probe, mate, nonmate)
        else:
            raise ValueError(f"Bad net name {net_name}!")
        img_saliency = cam1
        # </editor-fold>
    else:
        # # <editor-fold desc="[+] VGG16 ...">
        ...
        probe, mate, nonmate = param_package
        if net_name.lower() == "vgg16":
            cam1, hmap1 = xfr_fn.truncated_contrastive_triplet_ebp_vgg16_vmeval(probe, mate, nonmate)
        elif net_name.lower() == "lcnn9":
            cam1, hmap1 = xfr_fn.truncated_contrastive_triplet_ebp_lightcnn9_vmeval(probe, mate, nonmate)
        else:
            raise ValueError(f"Bad net name {net_name}!")
        img_saliency = cam1
        # </editor-fold>
...
def mean_ebp(wb, probe_im, net_name, ebp_version, device, param_package):
    # <editor-fold desc="[+] LCNN9 ...">
    probe, mate, nonmate = param_package
    cam1, hmap1 = xfr_fn.triplet_ebp_lightcnn9_vmeval(probe, mate, nonmate)
    img_saliency = cam1
    # </editor-fold>
...
def generate_wb_smaps(
    ...
    inpainting_v2_data = inpainting_v2_data.loc[
        ...
        (inpainting_v2_data['SUBJECT_ID'] == int(subject_id))  # UoM
        # (inpainting_v2_data['SUBJECT_ID'] == subject_id)  # cuhk
    ]
    ...
    for probe_idx, (
        ...
        if method is None or method=='meanEBP':
            ...
            ebp_version, ebp_subtree_mode, device = net_param_package
            wb = create_wbnet(
                net_name,
            ...
            create_save_smap(
                ...
                smap_fn=lambda: mean_ebp(
                    ...
                    ebp_version=ebp_ver,
                    device=device,
                    param_package=(probes[0], mates[0], nonmates[0])
                ),
                probe_im=probe_im,
