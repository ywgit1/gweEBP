...
def classified_as_inpainted_twin(
    ...
    original_imT = original_imT.astype(np.float32) # Save memory cost
    inpaint_imT = inpaint_imT.astype(np.float32) # Save memory cost
    rgb_masks = rgb_masks.astype(np.float32) # Save memory cost
    blends = (
